...
function getIssues(req, res){
    ...
    if(tmp == null || tmp == ''){
        ...
        var queryStatement = 'SELECT id, project_name, priority, status, type, owner_id, developer_id, tester_id, DATE_FORMAT(create_date, "%Y-%m-%d") AS create_Date, DATE_FORMAT(due_date, "%Y-%m-%d") AS due_Date  FROM issue' ;
...
var issue_field = ['project name', 'title', 'priority', 'status', 'type', 'owner', 'developer', 'tester', 'create date', 'due date'];
